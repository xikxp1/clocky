...
struct SettingsView: View {
    ...
    private var familyBinding: Binding<String> {
        Binding(
            ...
            set: { family in
                settings.update { $0.fontName = family.isEmpty ? nil : fonts.defaultFace(in: family) }
            }
        )
    }

    private func binding<Value>(_ keyPath: WritableKeyPath<ClockPreferences, Value>) -> Binding<Value> {
        Binding(
    ...
    private func colorBinding(_ keyPath: WritableKeyPath<ClockPreferences, RGBAColor>) -> Binding<Color> {
        Binding(
    ...
    private var settingsContent: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            ClockView(text: time.text, preferences: settings.preferences, isPositioning: false)
                .frame(height: 120)
                .padding(10)
                .background(Color(nsColor: .underPageBackgroundColor), in: RoundedRectangle(cornerRadius: 16))
                .accessibilityLabel("Clock preview")
            ...
            Form {
                ...
                HStack {
                    Slider(value: binding(\.backgroundOpacity), in: 0...1) { Text("Background opacity") }
    ...
    private var fontControls: some View {
        ...
        if let family = fonts.family(for: settings.preferences.fontName) {
            Picker("Font face", selection: Binding(
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let reason = login.unavailableReason {
                ...
                if login.requiresApproval {
                    Text("Approval required: allow Clocky in macOS Login Items before it can start automatically.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    ...
                    Text("Keep Clocky.app in a stable location, such as Applications, before enabling this option.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
    ...
    init(settings: SettingsStore, overlays: OverlayManager, time: TimeService, login: LoginItemManager) {
        self.time = time
        self.login = login
        let view = SettingsView(settings: settings, overlays: overlays, time: time, login: login)
    ...
    func windowWillClose(_ notification: Notification) { time.setPreviewVisible(false) }
    func windowDidMiniaturize(_ notification: Notification) { time.setPreviewVisible(false) }
    func windowDidDeminiaturize(_ notification: Notification) { time.setPreviewVisible(true) }
}
